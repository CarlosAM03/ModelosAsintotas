...
function validate(p: HistoryParameters): void {
  ...
  if (!flat.every(Number.isFinite) || p.domain.start !== 0 || p.domain.end !== 1 || !(p.milestones.encounter > 0 && p.milestones.encounter < p.milestones.peak && p.milestones.peak < p.milestones.divergence && p.milestones.divergence < p.milestones.present && p.milestones.present < 1) || p.oscillation.initialAmplitude <= p.oscillation.minimumAmplitude || p.oscillation.minimumAmplitude <= 0 || p.oscillation.finalAmplitude <= p.oscillation.minimumAmplitude || p.memory.decay <= 0 || p.memory.residual <= 0) throw new Error("Invalid history parameters");
}
