...
export function observeAutoplay(element: HTMLElement, controller: AnimationController, threshold: number): void { if (!("IntersectionObserver" in window)) { controller.play(); return; } let activated = false; const observer = new IntersectionObserver(entries => { if (!activated && entries.some(e => e.isIntersecting)) { activated = true; controller.play(); observer.disconnect(); } }, { threshold }); observer.observe(element); }
